...
import static com.inlinegroup.vrcalculationbackend.exceptions.GlobalErrorAttributes.RESPONSE_PARAM_STATUS;
...
public class GlobalErrorExceptionHandler extends AbstractErrorWebExceptionHandler {
    ...
    private Mono<ServerResponse> renderErrorResponse(ServerRequest request) {
        ...
        Object status = errorPropertiesMap.get(RESPONSE_PARAM_STATUS);
        HttpStatus currentHttpStatus = HttpStatus.INTERNAL_SERVER_ERROR;
        if (status instanceof HttpStatus st) {
            currentHttpStatus = st;
        }
        return ServerResponse.status(currentHttpStatus)
